...
def get_model(p, pretrain_path=None):
    # Get backbone
    if p['backbone'] == 'resnet18':
        if p['train_db_name'] in ['cifar-10', 'cifar-100','cifar-balanced']:
...
def get_train_dataset(p, transform, to_augmented_dataset=False,
    ...
    elif p['train_db_name'] == 'cifar-balanced':
        from data.cifar import BalancedCIFAR10
        dataset = BalancedCIFAR10(train=True, transform=transform, download=True)
...
def get_val_dataset(p, transform=None, to_neighbors_dataset=False):
    ...
    elif p['val_db_name'] == 'cifar-balanced':
        from data.cifar import BalancedCIFAR10
        dataset = BalancedCIFAR10(train=True, transform=transform, download=True)
