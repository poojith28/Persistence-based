...
class TinyImageNet(datasets.VisionDataset):
    ...
    def __init__(self, root: str, split: str = 'train', transform=None, **kwargs: Any) -> None:
        ...
        if self.split == 'train':
            self.images, self.targets, self.cls_to_id = unpickle_object('/vast/s219110279/TypiClust_3/scan/datasets/tiny-imagenet/train.pkl')
        elif self.split == 'val':
            self.images, self.targets, self.cls_to_id = unpickle_object('/vast/s219110279/TypiClust_3/scan/datasets/tiny-imagenet/val.pkl')
        else:
