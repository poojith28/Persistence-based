...
class BalancedCIFAR10(Dataset):
    """`Balanced CIFAR10` Dataset with 500 samples per class for training.
    Args:
        root (string): Root directory of dataset where directory
            ``cifar-10-batches-py`` exists or will be saved to if download is set to True.
        train (bool, optional): If True, creates dataset from training set, otherwise
    ...
    def __init__(self,  root=MyPath.db_root_dir('cifar-10'), train=True, transform=None, download=False):
        super(BalancedCIFAR10, self).__init__()
        self.root = root
        self.transform = transform
        self.train = train  # training set or test set
        self.classes = ['plane', 'car', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck']
    ...
    def _balance_dataset(self):
        """Randomly balance the dataset to have 500 samples per class."""
