"""
persistence_cifar100_normalised.py
──────────────────────────────────
Build a Vietoris–Rips persistence diagram for ALL CIFAR-100 images after
NORMALISING the data set (no PCA, no sub-sampling).

Normalisation modes
-------------------
--norm pixel      : values ∈ [0,1]   (default; torchvision.ToTensor())
--norm zscore     : per-pixel standard score  (x - μ) / σ
--norm unit       : per-sample L2-normalise each image vector  ||x||₂ = 1

USAGE EXAMPLES
==============
# 1) z-score normalisation, full train split, H₁ only
python persistence_cifar100_normalised.py --root ~/data --norm zscore

# 2) unit-sphere normalisation, test split, up to H₂
python persistence_cifar100_normalised.py --root ~/data --split test \
        --norm unit --max_rips_dim 2
"""
...
logging.basicConfig(
    ...
    filename="cifar100_normalised_persistence.log",
    filemode="w"
)
...
# ❸  Data loading
# ───────────────────────────────────────────────────────────────────────────
def load_cifar100_flat(root: str, split: str = "train") -> np.ndarray:
    """
...
def main(args):
    # 1. load
    logging.info("Loading CIFAR-100 (%s split)…", args.split)
    X = load_cifar100_flat(args.root, split=args.split)
...
if __name__ == "__main__":
    ...
    p.add_argument("--root", type=str, default="./data",
                   help="Folder where torchvision downloads CIFAR-100")
    ...
    p.add_argument("--csv", type=str, default="cifar100_normalised_pairs_RAWDATA.csv",
